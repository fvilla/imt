    ...
    public void rasterize(FeatureIterator<SimpleFeature> fc, java.awt.geom.Rectangle2D.Double bounds, 
        ...
        while (fc.hasNext()) {       
        	try {
        		...
        		Geospace.get().warn("problem reading feature #" + n + ": " + e.getMessage());
        	}
        	// log when we have to rasterize lots of features, so we know we should take action otherwise 
        	if (n > 0 && (n % 5000) == 0)
        		Geospace.get().info("rasterized " + n + "-th feature... that's a lot to rasterize");
        }
        
        close();

        Geospace.get().info("rasterized " + n + " features");
