...
public class Geospace extends ThinklabActivator {
...
//	}
	...
	private IGazetteer readGazetteerFromDirectory(File gdir) throws ThinklabException {
		...
		if (props.exists()) {
			...
			info("loading gazetteer from " + gdir);
			...
			for (String ff : gdir.list()) {
				if (ff.endsWith(".shp")) {
					...
					if (!lck.exists()) {
						...
						info("adding gazetteer source " + ff);
	...
	public static CoordinateReferenceSystem chooseCRS(
		...
		CoordinateReferenceSystem ret = get().getPreferredCRS();
		
		if (ret == null) {
			ret = crs1;
			get().setPreferredCRS(ret);
		}
