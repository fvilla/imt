...
import org.integratedmodelling.thinkcap.ZK;
...
public class Sidebar extends ThinkcapComponent {
	...
	ThinkcapSession session = null;
	ThinkcapApplication application = null;

	public Sidebar() {
		super();
		// get these while we are in the main thread.
		session = Thinkcap.getThinkcapSession(Sessions.getCurrent());
	...
	public class ModuleHeader {
		...
		public void redisplay() {
			...
			if (_current != null) {
				...
				if (rightImage != null)
					rt.appendChild(ZK.image(rightImage).tmargin(2).rmargin(2).get());
			}
		}
	...
	public void setWidth(String width) {
		if (!width.endsWith("px")) {
			throw new ThinkcapRuntimeException("sidebar height must be given in explicit pixels");
		}
	...
	private int[] computeSizes() {
		...
		if (sHeight < 0 || sWidth < 0) {
			throw new ThinkcapRuntimeException("sidebar dimensions must be explicit");			
		}
